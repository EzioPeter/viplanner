...
from omni.viplanner.utils import VIPlannerMatterportRayCasterCamera
...
def matterport_raycast_camera_data(env: BaseEnv, sensor_cfg: SceneEntityCfg, data_type: str) -> torch.Tensor:
    ...
    # return the data
    if "distance_to_image_plane" == data_type:
...
def isaac_camera_data(env: BaseEnv, sensor_cfg: SceneEntityCfg, data_type: str) -> torch.Tensor:
    ...
    if "distance_to_image_plane" == data_type:
        ...
        output[torch.isinf(output)] = 0.0
        return output
    elif "semantic_segmentation" == data_type:
        # retrive data
        ...
        info = [{int(k): VIPLANNER_SEM_META.class_color["static"] if v["class"] in ('BACKGROUND', 'UNLABELLED') else VIPLANNER_SEM_META.class_color[v["class"]] for k, v in d.items()} for d in info]
        ...
        for env_id in range(env.num_envs):
            ...
            mapping[list(info[env_id].keys())] = torch.tensor(list(info[env_id].values()), dtype=torch.uint8, device=env.device)
