...
def main(args):
    ...
    """
    print(
        f"Extract images from {args.bag_file} on topic {args.image_topic} into"
        f" {args.output_dir}"
    )
    ...
    if args.nb_images:
        ...
        ), "Should select {} but only total of {} included in bag!".format(
            args.nb_images,
            message_count,
        )
    ...
    for topic, msg, t in bag.read_messages(topics=[args.image_topic]):
        if count in image_idx:
            ...
            cv2.imwrite(
                os.path.join(args.output_dir, "frame%06i.jpg" % count), cv_img
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-bf",
        "--bag_file",
        default="/home/pascal/viplanner/env/anymal/2023_01_26_eth/2023-01-26-16-10-24_rgb_front_rear_lidar_depth_left_right_mission_3.bag",  # required=True,
        help="Input ROS bag.",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        default="/home/pascal/viplanner/env/anymal/2023_01_26_eth/2023-01-26-16-10-24_rgb_front_rear_lidar_depth_left_right_mission_3",  # required=True,
        help="Output directory.",
    )
    parser.add_argument(
        "-t",
        "--image_topic",
        default="/wide_angle_camera_front/image_color_rect/compressed",  # required=True,
        help="Image topic.",
    )
    parser.add_argument(
        "-n",
