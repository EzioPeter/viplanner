...
class TrajViz:
    ...
    def VizTrajectory(self, preds, waypoints, odom, goal, fear, augment_viz, cost_map=True, visual_height=0.5, mesh_size=0.5, fov_angle: float = 0.0):
        ...
        fov_vis_pt_right = pp.SE3(odom) @ pp.SE3([fov_vis_length * np.cos(fov_angle/2), fov_vis_length * np.sin(fov_angle/2), 0, 0, 0, 0, 1])
        fov_vis_pt_left = pp.SE3(odom) @ pp.SE3([fov_vis_length * np.cos(fov_angle/2), -fov_vis_length * np.sin(fov_angle/2), 0, 0, 0, 0, 1])
