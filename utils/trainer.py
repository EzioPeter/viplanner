...
class Trainer:
    ...
    def _load_model(self, resume: bool = False) -> None:
        ...
        if resume:
            ...
            print("Resume train from {} with loss {}".format(self.model_path, self.best_loss))

        return
    
    def _configure_optimizer(self) -> None:
        if self._cfg.optimizer == "adam":
            self.optimizer = optim.Adam(self.net.parameters(), lr=self._cfg.lr, weight_decay=self._cfg.w_decay)
        elif self._cfg.optimizer == "sgd":
            self.optimizer = optim.SGD(self.net.parameters(), lr=self._cfg.lr, momentum=self._cfg.momentum, weight_decay=self._cfg.w_decay)
        else:
    ...
    def _train_epoch(
        ...
        for batch_idx, inputs in enumerater:
            ...
            enumerater.set_description(f"Epoch: {epoch} in Env: ({env_id+1}/{len(self._cfg.env_list)-1}) - train loss:{round(train_loss/(batch_idx+1), 4)} on {batch_idx}/{batches}")
