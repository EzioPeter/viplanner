...
# viplanner
from viplanner.config.costmap_cfg import Loader, CostMapConfig
...
class CostMapPCD:
    ...
    def setup(self):
        ...
        self.SetUpCostArray()
         # update pcd instance
        xv, yv = np.meshgrid(np.linspace(0, self.num_x * self.cfg.general.resolution, self.num_x), np.linspace(0, self.num_y * self.cfg.general.resolution, self.num_y), indexing="ij")
    ...
    def SaveTSDFMap(self):
        ...
        yaml_path = os.path.join(self.cfg.general.root_path, "maps", "params", f"config_{self.cfg.map_name}.yaml")
        with open(yaml_path, 'w+') as file:
            yaml.dump(vars(self.cfg), file, allow_unicode=True, default_flow_style=False)
    ...
    def ReadTSDFMap(cls, root_path: str, map_name: str, gpu_id: Optional[int] = None):
        ...
        return cls(cfg=cfg, tsdf_array=tsdf_array, viz_points=viz_points, ground_array=ground_array, gpu_id=gpu_id, load_from_file=True)
