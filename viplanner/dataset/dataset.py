...
class PlannerData(Dataset):
    def __init__(
        ...
        assert not (
            semantics and rgb
        ), "Semantics and RGB cannot be used at the same time"
    ...
    def _add_random_polygons(self, image, nb_polygons, max_size):
        for i in range(nb_polygons):
            ...
            polygon_points = np.random.randint(
                0, max_size, size=(num_corners, 2)
            )
    ...
    def load_data_in_memory(self) -> None:
        ...
        for idx in tqdm(
            range(len(self.depth_filename)), desc="Load images into RAM"
        ):
    ...
    def _load_depth_img(self, idx) -> torch.Tensor:
        if self.depth_filename[idx].endswith(".png"):
            ...
            if self._cfg.real_world_data:
                depth_image = np.array(
                    depth_image.transpose(PIL.Image.ROTATE_180)
                )
            else:
        ...
        if self._cfg.depth_salt_pepper or self._cfg.depth_gaussian:
            depth_norm = (depth_image - np.min(depth_image)) / (
            ...
            depth_image = depth_norm * (
                np.max(depth_image) - np.min(depth_image)
            ) + np.min(depth_image)
        if (
            self._cfg.depth_random_polygons_nb
            and self._cfg.depth_random_polygons_nb > 0
        ):
    ...
    def _load_sem_rgb_img(self, idx) -> torch.Tensor:
        ...
        if (
            ...
            and self._cfg.sem_rgb_random_polygons_nb > 0
        ):
        ...
        assert image.round(decimals=1).max() <= 1.0, (
            f"Image '{self.sem_rgb_filename[idx]}' is not normalized with max"
            f" value {image.max().item()}"
        )
...
class DistanceSchemeIdx:
    ...
    def get_data(
        ...
        assert (
            self.has_data
        ), f"DistanceSchemeIdx for distance {self.distance} has no data"
        ...
        if len(idx_fov) == 0:
            print(
                f"[WARNING] for distance {self.distance} no 'within_fov'"
                " samples"
            )
            ...
            if augment:
                idx_augment.append(
                    np.random.choice(
                        ...
                        replace=(
                            False
                            if nb_fov - len(idx_fov) < len(idx_fov)
                            else True
                        ),
                    )
                )
            else:
                idx_fov = np.random.choice(
                    idx_fov, len(idx_fov), replace=False
                )
        else:
        ...
        if len(idx_front) == 0:
            print(
                f"[WARNING] for distance {self.distance} no 'front_of_robot'"
                " samples"
            )
            ...
            if augment:
                idx_augment.append(
                    np.random.choice(
                        ...
                        replace=(
                            False
                            if nb_front - len(idx_front) < len(idx_front)
                            else True
                        ),
                    )
                )
                ...
                idx_front = np.random.choice(
                    idx_front, len(idx_front), replace=False
                )
        else:
        ...
        if len(idx_back) == 0:
            print(
                f"[WARNING] for distance {self.distance} no 'behind_robot'"
                " samples"
            )
            ...
            if augment:
                idx_augment.append(
                    np.random.choice(
                        ...
                        replace=(
                            False
                            if nb_back - len(idx_back) < len(idx_back)
                            else True
                        ),
                    )
                )
                ...
                idx_back = np.random.choice(
                    idx_back, len(idx_back), replace=False
                )
        else:
        ...
        if idx_augment:
            ...
            augment = np.hstack(
                [np.zeros(len(idx)), np.ones(len(idx_augment))]
            )
...
class PlannerDataGenerator(Dataset):
    ...
    def __init__(
        ...
        self.rgb = rgb
        assert not (
            self.semantics and self.rgb
        ), "semantics and rgb cannot be true at the same time"
    ...
    def load_odom(self) -> None:
        ...
        odom_path = os.path.join(
            self.root, f"camera_extrinsic{self._cfg.depth_suffix}.txt"
        )
        ...
        if self.semantics or self.rgb:
            odom_path = os.path.join(
                self.root, f"camera_extrinsic{self._cfg.sem_suffix}.txt"
            )
            odom_np = np.loadtxt(odom_path, delimiter=",")
    ...
    def load_images(self, root_path, domain: str = "depth"):
        ...
        assert os.path.isdir(img_path), (
            f"Image directory path '{img_path}' does not exist for domain"
            f" {domain}"
        )
        assert (
            len(os.listdir(img_path)) > 0
        ), f"Image directory '{img_path}' is empty for domain {domain}"
        ...
        if domain == "depth":
            img_filename_list.sort(
                key=lambda x: int(
                    x.split("/")[-1][: -(4 + len(self._cfg.depth_suffix))]
                )
            )
        else:
            img_filename_list.sort(
                key=lambda x: int(
                    x.split("/")[-1][: -(4 + len(self._cfg.sem_suffix))]
                )
            )
    ...
    def filter_obs_inflation(self) -> None:
        ...
        print(
            (
                "[INFO] Filter odom points within the inflation range of the"
                " obstacles in the cost map..."
            ),
            end="",
        )

        norm_inds, _ = self.cost_map.Pos2Ind(
            self.odom_array_depth[:, None, :3]
        )
        cost_grid = self.cost_map.cost_array.T.expand(
            self.odom_array_depth.shape[0], 1, -1, -1
        )
        ...
        if self.debug:
            ...
            for i in range(len(self.odom_array_depth)):
                if round(oloss_M[i].item(), 3) == 0.0:
                    small_sphere.paint_uniform_color(
                        [0.4, 0.1, 1.0]
                    )  # violette
        ...
        self.odom_array_depth = self.odom_array_depth[
            points_free_space.squeeze()
        ]
        ...
        self.depth_filename_list = [
            depth_filename_list[i]
            for i in range(len(depth_filename_list))
            if points_free_space[i]
        ]

        if self.semantics:
            self.odom_array_sem_rgb = self.odom_array_sem_rgb[
                points_free_space.squeeze()
            ]
            sem_rgb_filename_list = self.load_images(self.root, "semantics")
            self.sem_rgb_filename_list = [
                sem_rgb_filename_list[i]
                for i in range(len(sem_rgb_filename_list))
                if points_free_space[i]
            ]
        elif self.rgb:
            self.odom_array_sem_rgb = self.odom_array_sem_rgb[
        ...
        assert (
            len(self.depth_filename_list) != 0
        ), "No depth images left after filtering"
        print("DONE!")
        print(
    ...
    def compute_ratios(self) -> Tuple[float, float, float]:
        ...
        ratio_front = (
            np.sum(self.pair_front_of_robot) / self.odom_depth.shape[0]
        )
    ...
    def get_graph(self) -> None:
        ...
        if self.semantics or self.rgb:
            occupancy_map = (
                cost_array
                > self._cfg.obs_cost_height
                + abs(self.cost_map.cfg.sem_cost_map.negative_reward)
            ).astype(np.uint8)
        else:
            occupancy_map = (cost_array > self._cfg.obs_cost_height).astype(
                np.uint8
            )
        ...
        _, nearest_neighbors_idx = kdtree.query(
            odom_points, k=num_connections + 1, workers=-1
        )
        ...
        inter_points = np.stack(
            (x_interp.reshape(-1), y_interp.reshape(-1)), axis=1
        )
        # get the indices of the interpolated points in the occupancy map
        occupancy_idx = (
            inter_points
            - np.array([self.cost_map.cfg.x_start, self.cost_map.cfg.y_start])
        ...
        # get edge indices
        idx_edge_start = np.repeat(
            np.arange(odom_points.shape[0]), repeats=num_connections, axis=0
        )
        ...
        pos_attr = {
            i: {"pos": odom_points[i]} for i in range(odom_points.shape[0])
        }
        nx.set_node_attributes(self.graph, pos_attr)
        # add edges with distance attributes
        self.graph.add_edges_from(
            list(map(tuple, np.stack((idx_edge_start, idx_edge_end), axis=1)))
        )
        distance_attr = {
            (i, j): {
                "distance": np.linalg.norm(odom_points[i] - odom_points[j])
            }
    ...
    def get_pairs(self):
        ...
        if self.semantics or self.rgb:
            ...
            self.pix_depth_cam_frame = self.compute_pixel_tensor(
        ...
        self.category_scheme_pairs: Dict[float, DistanceSchemeIdx] = {
            distance: DistanceSchemeIdx(distance=distance)
            for distance in self._cfg.distance_scheme.keys()
        }

        # iterate over all odom points
        for odom_idx in tqdm(
            range(self.nb_odom_points), desc="Start-End Pairs Generation"
        ):
            ...
            if self.debug:
                ...
                fov_vis_length = (
                    0.75  # length of the fov visualization plane in meters
                )
        ...
        if self.debug:
            ...
            for distance in self._cfg.distance_scheme.keys():
                odoms = torch.vstack(
                    self.category_scheme_pairs[distance].odom_list
                )
    ...
    def reduce_pairs(
        ...
        distances = np.array(list(odom_distances.values()))[
            keep_distance_entries.numpy()
        ]
        goal_idx = np.array(list(odom_distances.keys()))[
            keep_distance_entries.numpy()
        ]
    ...
    def get_goal_categories(self, goal_odom_frame: pp.LieTensor):
        ...
        goal_angle = abs(
            torch.atan2(goal_odom_frame.data[:, 1], goal_odom_frame.data[:, 0])
        )
    ...
    def split_samples(
        ...
        ratio_fov_samples = (
            ratio_fov_samples
            if ratio_fov_samples is not None
            else self._cfg.ratio_fov_samples
        )
        ratio_front_samples = (
            ratio_front_samples
            if ratio_front_samples is not None
            else self._cfg.ratio_front_samples
        )
        ratio_back_samples = (
            ratio_back_samples
            if ratio_back_samples is not None
            else self._cfg.ratio_back_samples
        )
        assert (
            round(
                ratio_fov_samples + ratio_front_samples + ratio_back_samples, 2
            )
            == 1.0
        ), (
        ...
        for distance, distance_percentage in self._cfg.distance_scheme.items():
            if not self.category_scheme_pairs[distance].has_data:
                print(
                    f"[WARN] No samples for distance {distance} in ENV"
                    f" {os.path.split(self.root)[-1]}"
                )
                continue
            ...
            buffer_data = self.category_scheme_pairs[distance].get_data(
                nb_fov=int(
                    ratio_fov_samples * distance_percentage * max_sample_number
                ),
                nb_front=int(
                    ratio_front_samples
                    * distance_percentage
            ...
            augment_samples[current_idx : current_idx + nb_samples] = (
                buffer_data[4]
            )
        ...
        if generate_split:
            ...
            train_dataset.update_buffers(
                ...
                sem_rgb_filename=(
                    [sem_rgb_filename[i] for i in train_index]
                    if (self.semantics or self.rgb)
                    else None
                ),
        ...
        test_dataset.update_buffers(
            ...
            sem_rgb_filename=(
                [sem_rgb_filename[i] for i in idx]
                if (self.semantics or self.rgb)
                else None
            ),
            odom=odom[idx],
    ...
    @staticmethod
    def compute_pixel_tensor(
        x_nums: int, y_nums: int, K_depth: np.ndarray
    ) -> None:
    ...
    def compute_overlay(
        ...
        points = (
            dep_im_reshaped * (rot @ pix_depth_cam_frame.T).T + pose_dep[:3]
        )
    ...
    def _get_overlay_img(self, odom_idx):
        ...
        if self._cfg.real_world_data:
            sem_rgb_image = np.array(
                sem_rgb_image.transpose(PIL.Image.ROTATE_180)
            )
        else:
        ...
        assert (
            sem_rgb_image_warped.dtype == np.uint8
        ), "sem_rgb_image_warped has to be uint8"
        ...
        sem_rgb_image_path = os.path.join(
            self.root, "img_warp", sem_rgb_filename
        )
        sem_rgb_image_warped = cv2.cvtColor(
            sem_rgb_image_warped, cv2.COLOR_RGB2BGR
    ...
    def noise_edges(self):
        ...
        for depth_filename in self.depth_filename_list:
            ...
            image = ((depth_img / depth_img.max()) * 255).astype(
                np.uint8
            )  # convert to CV_U8 format
            edges = cv2.Canny(
                image, self._cfg.edge_threshold, self._cfg.edge_threshold * 3
            )
            ...
            if depth_filename.endswith(".png"):
                assert cv2.imwrite(
                    os.path.join(
                        depth_noise_edge_dir, os.path.split(depth_filename)[1]
                    ),
                    depth_img,
                )
            else:
            ...
            new_depth_filename_list.append(
                os.path.join(
                    depth_noise_edge_dir, os.path.split(depth_filename)[1]
                )
            )
    ...
    def cleanup(self):
        print(
            (
                "[INFO] Cleaning up for environment"
                f" {os.path.split(self.root)[1]} ..."
            ),
            end=" ",
        )
