...
import pickle
waypoints = pickle.load(open("/home/pascal/viplanner/imperative_learning/data/waypoints/explored_warehouse_multiple_shelves_without_ppl_ext_sem_space_seed1_pairs500.pkl", "rb"))
goals = []
...
for idx, curr_dataset in enumerate(datasets):
    goal_reached = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_goal_reached.npy"))
    goal_within_fov = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_goal_within_fov.npy"))
    base_collision = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_base_collision.npy"))
    knee_collision = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_knee_collision.npy"))
    walking_time = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_walking_time.npy"))
    goal_distances = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_goal_distances.npy"))
    length_goal = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_length_goal.npy"))
    length_path = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_length_path.npy"))
    loss_obstacles_max_sem = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_loss_obstacles.npy"))
    skip_waypoint = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_skip_waypoint.npy"))
    path_extension = np.load(os.path.join(model, curr_dataset, prefixs[idx] + f"_path_extension.npy"))
    ...
    for waypoint_nbr in range(len(loss_obstacles_max_sem)):
        ...
        if "warehouse" in costmap_dirs[idx]:
            goes_through_shelf = np.where((curr_waypoint[:, 0] < 0.2) & (curr_waypoint[:, 0] > -0.9) & (curr_waypoint[:, 1] > -3.6) & (curr_waypoint[:, 1] < 12.56))[0]
    ...
    if "warehouse" in costmap_dirs[idx]:
        filter_idx = [32, 44, 48, 62, 70, 77, 90, 94, 117, 124, 125, 141, 143, 146, 149, 165, 167, 174, 176, 187, 197, 202, 206, 216, 218, 225, 228, 233, 238, 240, 241, 244, 248, 249, 258, 261, 262, 267, 273, 278, 290, 294, 296, 298, 305, 307, 315, 318, 320, 327, 328, 333, 334, 336, 339, 340, 346, 348, 350, 352, 354, 357, 361, 363, 364, 366, 367, 368, 370, 371, 375, 376, 377, 379, 380, 383, 385, 387, 392, 393, 395, 398, 399, 400, 402, 403, 405, 406, 416, 417, 423, 425, 427, 432, 433, 438, 445, 448, 450, 451, 453, 454, 457, 465, 467, 470, 472, 473, 475, 476, 483, 485, 487, 488, 490, 32, 33, 335, 336, 353, 354, 427, 446, 452, 469, 471, 298, 301, 296, 482, 489,  391, 397, 398, 213, 215, 218]
