    ...
    def _sep_data(self) -> None:
        ...
        for idx, goal_time in enumerate(self.time_goal[:-1]):
            if idx == len(self.time_goal) - 1:
                ...
                path_masks[idx] = np.logical_and(
                    self.time_base >= goal_time,
                    self.time_base <= self.time_goal[idx + 1],
                )
        return path_masks
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Model Eval",
        description="Evaluate the VIPlanner paths walked in the real-world",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        help="Path to data directory (should contain goal and odom data)",
