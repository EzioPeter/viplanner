...
class RealWorldEvaluator(BaseEvaluator, RealWorldDataHandler):
    ...
    def run_model(self, model_dir: str) -> None:
        ...
        if train_config.sem:
            ...
            m2f_wrapper.run_on_folder(
                os.path.join(self.args.data_dir, "bgr"),
                self.args.debug,
                run_on_existing_files=False,
            )
        ...
        depth_img = cv2.imread(
            os.path.join(self.args.data_dir, "depth", self.depth_img_list[0]),
            cv2.IMREAD_ANYDEPTH,
        )
        ...
        for idx, img in enumerate(
            tqdm(
                self.depth_img_list[: -max(self.args.goal_frame_advances)],
                desc="VIPlanner Predictions",
            )
        ):
            # load images
            depth_img = cv2.imread(
                os.path.join(self.args.data_dir, "depth", img),
                cv2.IMREAD_ANYDEPTH,
            )
            ...
            if train_config.sem or train_config.rgb:
                ...
                bgr_sem_img_warp = PlannerDataGenerator.compute_overlay(
                    ...
                    self.odom_bgr[idx],
                    depth_img=depth_img,
                    sem_rgb_image=bgr_sem_img,
                    pix_depth_cam_frame=depth_pixel_array,
                    K_sem_rgb=self.K_bgr,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Model Eval",
        description="Evaluate VIPmodels with real-world data",
    )
    parser.add_argument(
        "-m",
        "--model_dirs",
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="Number of frames to advance the goal",
