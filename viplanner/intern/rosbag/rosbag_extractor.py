...
def main(args):  # noqa: C901
    ...
    pbar = tqdm.tqdm(
        total=(
            bag.get_message_count(args.topic_depth)
            + bag.get_message_count(args.topic_bgr)
            + bag.get_message_count(args.topic_state)
            + bag.get_message_count(args.topic_goal)
            + bag.get_message_count(args.topic_path)
            + bag.get_message_count(args.topic_sem)
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-td",
        "--topic_depth",
        default="/depth_cam_mounted_front/depth/image_rect_raw",  # '/depth_camera_front_upper/depth/image_rect_raw',  # '/depth_camera_front/depth/image_rect_raw'
        help="Image topic.",
    )
    parser.add_argument(
        "-tr",
