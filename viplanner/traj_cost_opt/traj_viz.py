...
class TrajViz:
    ...
    def VizTrajectory(
        ...
        preds_ws = (
            TrajCost.TransformPoints(odom, preds)
            .tensor()
            .cpu()
            .detach()
            .numpy()
        ...
        wp_ws = (
            TrajCost.TransformPoints(odom, waypoints)
            .tensor()
            .cpu()
            .detach()
            .numpy()
        ...
        mesh_sphere_augment = o3d.geometry.TriangleMesh.create_sphere(
            mesh_size / 1.5
        )  # start points
        small_sphere = o3d.geometry.TriangleMesh.create_sphere(
            mesh_size / 3.0
        )  # successful trajectory points
        small_sphere_fear = o3d.geometry.TriangleMesh.create_sphere(
            mesh_size / 3.0
        )  # unsuccessful trajectory points
        ...
        for i in range(batch_size):
            ...
            fov_mesh = o3d.geometry.TriangleMesh(
                vertices=o3d.utility.Vector3dVector(
                    np.array([op[:3], fov_vis_pt_right[i], fov_vis_pt_left[i]])
                ),
            ...
            if augment_viz[i]:
                visual_list.append(
                    copy.deepcopy(mesh_sphere_augment).translate(
                        (op[0], op[1], op[2])
                    )
                )
            else:
                visual_list.append(
                    copy.deepcopy(mesh_sphere).translate((op[0], op[1], op[2]))
                )
            visual_list.append(
            ...
            for j in range(preds_ws[i].shape[0]):
                ...
                if fear[i, :] > 0.5:
                    visual_list.append(
                        copy.deepcopy(small_sphere_fear).translate(
                            (kp[0], kp[1], kp[2] + visual_height)
                        )
                    )
                else:
                    visual_list.append(
    ...
    def VizImages(
        ...
        mesh_sphere = o3d.geometry.TriangleMesh.create_sphere(
            mesh_size / 2.0
        )  # successful predict points
        mesh_sphere_fear = o3d.geometry.TriangleMesh.create_sphere(
            mesh_size / 2.0
        )  # unsuccessful predict points
        mesh_box = o3d.geometry.TriangleMesh.create_box(
            mesh_size, mesh_size, mesh_size * 2
        )  # end points
        # set colors
        ...
        for i in range(batch_size):
            ...
            for j, kp in enumerate(preds_ws[i]):
                if fear[i, :] > 0.5:
                    kp_mesh = copy.deepcopy(mesh_sphere_fear).translate(
                        (kp[0], kp[1], kp[2] - visual_offset)
                    )
                else:
                    kp_mesh = copy.deepcopy(mesh_sphere).translate(
                        (kp[0], kp[1], kp[2] - visual_offset)
                    )
            ...
            for k, wp in enumerate(wp_ws[i]):
                if k < wp_start_idx:
                    continue
                wp_mesh = copy.deepcopy(small_sphere).translate(
                    (wp[0], wp[1], wp[2] - visual_offset)
                )
        ...
        tilt_vec.extend(
            list(
                tf.Rotation.from_euler(
                    "y", self._camera_tilt, degrees=False
                ).as_quat()
            )
        )
