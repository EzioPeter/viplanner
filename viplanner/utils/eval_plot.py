...
from viplanner.utils.rosbag_base_handler import RealWorldDataHandler, ROS_TO_ROBOTICS_MAT, CAMERA_FLIP_MAT
...
class EvalPlotter(RealWorldDataHandler):
    ...
    def load_data(self):
        ...
        path_depth = (path[path_idx, :, :3] - self.odom_depth[img_idx, None, :3]) @ tf.Rotation.from_quat(self.odom_depth[img_idx, 3:]).as_matrix()
        path_rgb   = (path[path_idx, :, :3] - self.odom_bgr[img_idx, None, :3]) @ tf.Rotation.from_quat(self.odom_bgr[img_idx, 3:]).as_matrix()
        ...
        for idx in range(len(self.path_depth)):
            if (idx == goal_switch_idx[1]).any():
                ...
                continue
            self.goal_depth[idx] = (goals[goal_idx, :3] - self.odom_depth[idx, :3].numpy()) @ tf.Rotation.from_quat(self.odom_depth[idx, 3:].numpy()).as_matrix()
            self.goal_rgb[idx] = (goals[goal_idx, :3] - self.odom_bgr[idx, :3].numpy()) @ tf.Rotation.from_quat(self.odom_bgr[idx, 3:].numpy()).as_matrix()
        ...
        if self.semantics:
            ...
            self.sem_img      = torch.zeros((len(self.sem_img_list), self.bgr_img_shape[2], self.bgr_img_shape[0], self.bgr_img_shape[1]))

        # load all images
        self.depth_img = torch.zeros((len(self.depth_img_list), self.depth_img_shape[0], self.depth_img_shape[1]))
        self.bgr_img   = torch.zeros((len(self.bgr_img_list), self.bgr_img_shape[2], self.bgr_img_shape[0], self.bgr_img_shape[1]))
        for idx in range(len(self.depth_img_list)):
            self.depth_img[idx] = torch.from_numpy(cv2.imread(os.path.join(self.dir, "depth", self.depth_img_list[idx]), cv2.IMREAD_UNCHANGED)  / 1000.0) # rotate by 180 deg
