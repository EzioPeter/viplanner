...
class BaseEvaluator:
    def __init__(
        ...
        self.obs_loss_theshold = obs_loss_threshold
    ...
    def eval_statistics(self) -> None:
        ...
        print(
            ...
            f"Reached goal rate (thres: {self.distance_tolerance}):\t{goal_reached_rate} \n"
            f"Avg goal-distance (all):    \t{avg_distance_to_goal} \n"
            f"Avg goal-distance (reached):\t{avg_distance_to_goal_reached}"
        )
        ...
        if self._use_cost_map:
            within_obs_threshold = np.sum(self.loss_obstacles < self.obs_loss_theshold) / len(self.loss_obstacles)
            ...
            print(
                f"Within obs threshold ({self.obs_loss_theshold}):\t{within_obs_threshold} \n"
                f"Obstacle loss (all):        \t{avg_obs_loss} \n"
                f"Obstacle loss (reached):    \t{avg_obs_loss_reached} \n"
                f"Max obstacle loss (all):    \t{max_obs_loss} \n"
                f"Max obstacle loss (reached):\t{max_obs_loss_reached}"
            )
    ...
    def plt_single_model(self, eval_dir: str, show: bool = True) -> None:
        ...
        ax.plot(unique_goal_length, mean_path_extension, color='blue', label='Average path length')
        ax.fill_between(unique_goal_length, np.array(mean_path_extension) - np.array(std_path_extension), np.array(mean_path_extension) + np.array(std_path_extension), color='blue', alpha=0.2, label='Uncertainty')
        ax.set_xlabel('Start-Goal Distance', fontsize=16)
        ax.set_ylabel('Path Length', fontsize=16)
        ax.set_title(f"Avg increase of path length is {round(np.mean(self.path_extension), 5)*100:.2f}% for successful paths with tolerance of {self.distance_tolerance}", fontsize=16)
        ...
        ax1.set_title(f"With a tolerance of {self.distance_tolerance} are {round(goal_success_mean, 5)*100:.2f} % of goals reached", fontsize=16)
        ...
        if self._use_cost_map:
            ...
            obs_threshold_success_rate = np.sum(self.loss_obstacles < self.obs_loss_theshold) / len(self.loss_obstacles)

            fig, ax = plt.subplots(figsize=(12, 10))
            fig.suptitle("Obstacle Loss", fontsize=20)
            ax.plot(unique_goal_length, mean_obs_loss, color='blue', label='Average obs loss')
            ax.fill_between(unique_goal_length, np.array(mean_obs_loss) - np.array(std_obs_loss), np.array(mean_obs_loss) + np.array(std_obs_loss), color='blue', alpha=0.2, label='Uncertainty')
            ax.set_xlabel('Start-Goal Distance', fontsize=16)
            ax.set_ylabel('Obstacle Loss', fontsize=16)
            ax.set_title(f"Avg obstacle loss {round(avg_obs_loss, 5):.5f} with {obs_threshold_success_rate}% within obs thres {self.obs_loss_theshold}", fontsize=16)
            ax.tick_params(axis='both', which='major', labelsize=14)
            ax.legend()
    ...
    def plt_comparison(
        ...
        for idx in range(len(length_goal_list)):
            ...
            axs_path.bar(unique_goal_length + bar_pos, mean_path_extension, width=bar_width, label=f'{model_name} (avg {round(avg_increase, 5)*100:.2f} %))', alpha=0.8)  # yerr=std_path_extension, 
        ...
        axs_goal.axhline(y=self.distance_tolerance, color='red', linestyle='--', label="threshold")
