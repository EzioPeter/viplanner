...
# viplanner
from viplanner.traj_cost_opt import TrajViz, TrajCost
from viplanner.traj_cost_opt.traj_opt import TrajOpt
from viplanner.utils.rosbag_base_handler import ROS_TO_ROBOTICS_MAT
...
class EvalVideo:
    ...
    def __init__(
        ...
        self.depth_img_shape    = np.shape(cv2.imread(os.path.join(self.dir, "depth", self.depth_img_list[0]), cv2.IMREAD_UNCHANGED))
    ...
    def plot(self):
        ...
        rgb_idx, depth_idx = RealWorldDataHandler.synchronize_data(self.odom_depth[:, 7:], self.odom_rgb[:, 7:], threshold=1.0)
        ...
        for idx, path in enumerate(self.path):
            ...
            if idx == len(self.path) - 1:
                ...
                depth_idx = np.where((depth_time >= path_time[idx]) & (depth_time <= path_time[idx+1]) & (depth_time <= path_time[idx]+2))[0]  # 
            ...
            if len(depth_idx) > 0:
                ...
                curr_depth_quat = tf.Rotation.from_matrix(tf.Rotation.from_quat(self.odom_depth[depth_idx, 3:7]).as_matrix() @ ROS_TO_ROBOTICS_MAT).as_quat()
                ...
                for local_idx, curr_depth_idx in enumerate(depth_idx):
                    depth_img[local_idx] = torch.from_numpy(cv2.imread(os.path.join(self.dir, "depth", self.depth_img_list[curr_depth_idx]), cv2.IMREAD_UNCHANGED)  / 1000.0) # rotate by 180 deg
                ...
                for local_idx, curr_depth_idx in enumerate(depth_idx):
                    assert cv2.imwrite(os.path.join(self.dir, "video_depth_projected", self.depth_img_list[curr_depth_idx]), projected_img_depth[local_idx])
            ...
            if idx == len(self.path) - 1:
                ...
                rgb_idx = np.where((rgb_time >= path_time[idx]) & (rgb_time <= path_time[idx+1]) & (rgb_time <= path_time[idx]+2))[0]

            if len(rgb_idx) > 0:
                curr_rgb_quat = tf.Rotation.from_matrix(tf.Rotation.from_quat(self.odom_rgb[rgb_idx, 3:7]).as_matrix() @ ROS_TO_ROBOTICS_MAT).as_quat()

                path_odom = path[None, :, :3].repeat(len(rgb_idx), axis=0)
                goal_odom = self.odom_goal[None, goal_idx, :3].repeat(len(rgb_idx), axis=0)

                rgb_img = torch.zeros((len(rgb_idx), self.bgr_img_shape[2], self.bgr_img_shape[0], self.bgr_img_shape[1]))
                sem_img = torch.zeros((len(rgb_idx), self.bgr_img_shape[2], self.bgr_img_shape[0], self.bgr_img_shape[1]))
                for local_idx, curr_rgb_idx in enumerate(rgb_idx):
                    ...
                    rgb_img[local_idx]  = torch.from_numpy(cv2.cvtColor(rgb_img_curr, cv2.COLOR_BGR2RGB)).permute(2, 0, 1) / 255.0
                    ...
                    sem_img[local_idx]  = torch.from_numpy(cv2.cvtColor(rgb_sem_img, cv2.COLOR_BGR2RGB)).permute(2, 0, 1) / 255.0
                ...
                for local_idx, curr_rgb_idx in enumerate(rgb_idx):
                    assert cv2.imwrite(os.path.join(self.dir, "video_rgb_projected", self.bgr_img_list[curr_rgb_idx]), projected_img_rgb[local_idx])
                    assert cv2.imwrite(os.path.join(self.dir, "video_sem_after_projected", self.bgr_img_list[curr_rgb_idx]), projected_sem_rgb[local_idx])
