...
def change_annotation(annotation_file: str, matching: int = 9):
    with open(os.path.join(coco_data_path, annotation_file), "r") as file:
    ...
    print(f"Selected {len(selected_images)} images out of {len(annotations_filelist)} images.")
    images_selected_idx = []
    ...
    json.dump(coco_train_json_reduced, open(os.path.join(coco_data_path, f"{path}_reduced{ending}"), "w"))
    print("done")
