...
class Timer:
    ...
    def toc(self, prefix=""):
        self.end()
        print(prefix + f"{self.duration:f}s = {1 / self.duration:f}Hz")
...
class MovAvg(nn.Module):
    ...
    def append(self, point):
        if len(self.nums) == 0:
            ...
            self.nums.pop(0)
        return F.conv1d(
            torch.tensor(self.nums, dtype=torch.float).view(1, 1, -1),
            self.weight,
        ).view(-1)
...
class ConvLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x[
            :,
            :,
            w : w + self.conv.kernel_size[0],
            h + self.conv.kernel_size[1] - 1,
        ] -= self.color
        x[:, :, w, h : h + self.conv.kernel_size[1]] -= self.color
        x[
            :,
            :,
            w + self.conv.kernel_size[0] - 1,
            h : h + self.conv.kernel_size[1],
        ] -= self.color
        return value
...
class CosineLoss(nn.CosineEmbeddingLoss):
    ...
    def forward(self, x, y):
        return super().forward(x, y, self.target) / 2
...
class PearsonLoss(nn.CosineEmbeddingLoss):
    ...
    def forward(self, x, y):
        x = x - x.mean()
        y = y - y.mean()
        return super().forward(x, y, self.target)
...
class RandomMotionBlur:
    ...
    def __call__(self, img):
        ...
        Returns:
            tensor: Random motion blurred image.
        """
        p = random.random()
        if p <= self.p[0]:
            return img
...
class CorrelationSimilarity(nn.Module):
    ...
    def forward(self, x, y):
        ...
        indices = torch.stack(
            (indices // self.input_size[1], indices % self.input_size[1]),
            dim=-1,
        )
...
class CorrelationLoss(Correlation):
    ...
    def __init__(self, input_size, reduce=True, accept_translation=True):
        super().__init__(input_size, accept_translation)
        self.reduce = reduce

    def forward(self, x, y):
        loss = (1 - super().forward(x, y)) / 2
